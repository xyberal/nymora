...
Each policy class (e.g., "Tier2 members," "Tier2-eligible vouchers") has its own Merkle accumulator — over Poseidon on the BLS12-381 scalar field, the circuit's hash in its pinned instance (§6.5; proposals 0033, 0034) — publishing only a root hash, and only to members. The fold is normative (proposal 0035): a leaf enters as itself, an interior node is the untagged 2-to-1 `Poseidon(left, right)`, and an empty position is the field's zero. No leaf or node tag is needed: the pinned sponge writes the input length into its capacity element, so the 2-element node function is structurally disjoint from every tagged leaf derivation, and depth is a fixed protocol constant, which closes the shift substitution a variable-depth tree would admit. **The current epoch's roots have no lookup endpoint at all** (proposal 0025): they reach members exclusively in the boundary broadcast (§11), and historical roots are served under §7's access grant:
...
Second, **accumulator depth is a protocol constant, not an agora parameter, and the constant is 32** (proposals 0030, 0032). One depth is fixed network-wide, because the membership path lives inside the standardized circuit (§6.5) and its length *is* the depth: a per-agora depth would be a per-agora proof shape — the fingerprinting §6.5 exists to prevent. The value is pinned on the measurement 0030 deferred to: an inclusion path costs 243 constraints per level, so a shallower tree would save a modest, linear, per-proof cost against a terminal, unbounded, per-class risk — the asymmetry 0032 prices in full. What no constant changes is that capacity consumption is permanent: migrated predecessors and revoked members consume capacity forever, and since planned migration is the expected path for a routine device change, consumption tracks device churn rather than recruitment.

**Exhaustion is terminal for the class.** A policy class whose accumulator is full admits no further leaf under the current protocol version — no new admission and, because migration consumes capacity, no routine device change either, which is the worse of the two. No mechanism compacts, extends, or re-accumulates a tree: leaves cannot be withdrawn (above), and a re-accumulation ceremony would change every member's witness and the class's root lineage at once — a protocol-version event requiring its own proposal, not an operational recovery. The constant is fixed generously for exactly this reason: depth 32 accommodates roughly four billion leaves per class at thirty-two siblings per witness — sized past the largest class the network will ever host, because every class in every agora lives inside it and there is no second chance later.

### 5.3 Vouching protocol

Admission of a new candidate requires k-of-n independent zero-knowledge attestations from existing eligible members (k ≥ 1 — zero is malformed; §4.3, proposal 0027):
...
A fixed-shape, non-interactive zero-knowledge proof under the protocol's standardized proving system — **Plonkish arithmetization with KZG polynomial commitments over the BLS12-381 pairing curve**, under a universal updatable reference string inherited from an existing large ceremony — Filecoin's phase-1 powers of tau, its committed excerpt carried in the repository with the provenance chain and checksums that tie it to the ceremony transcript (proposals 0033, 0034) — rather than a setup of this protocol's own, using **one standardized circuit shared across every agora** — deliberately, so that proof size and structure never vary by agora, preventing proof-shape fingerprinting from correlating content back to a specific group. The proving field fixes the primitives with it: every `Hash(...)` a statement computes is **Poseidon over the BLS12-381 scalar field**, in one pinned instance — width 3 (rate 2, capacity 1), α = 5, 8 full and 60 partial rounds, its constants Grain-generated by the reproducible invocation proposal 0034 records — and the certificate scheme the chain verifies is **EdDSA over Jubjub with a Poseidon transcript**, stated as an equation in §9.1 (proposal 0034). The accumulator depth the membership path quantifies over is part of that shape, and is therefore a network-wide protocol constant (§5.2, proposal 0030).

**How bytes cross into the field is itself normative** (proposal 0035), because two implementations that disagree about it derive different nullifiers from identical state. A 32-byte identifier — `agora_id`, `message_hash`, a session context — enters by little-endian interpretation with bits 254 and 255 cleared. A variable-length identifier — a session, proposal, or challenge string — is first compressed by the byte family under `nymora/v0/action-context`, length-framed as always, and the digest enters by the same rule. An epoch number enters as the u64 injected. A field element leaves as its canonical 32-byte little-endian representation, and a non-canonical string names no value. Secret keys are minted below their moduli by the same device: 32 bytes of fresh entropy with the top two bits cleared for a field-element secret, the top five for a Jubjub scalar — which is what makes §9.1's canonicity clause true by construction for honestly minted keys.
...
**The signed message is canonical, and normative** (proposal 0035): the payload compresses to one element of the proving field by the pinned hash, and the compression *is* the message —
...
with `EPOCH_CERT` and `MIGRATION_CERT` from the field-domain registry, the epoch entering as the u64 injected, the `agora_id` by the identifier rule (§6.5), and the certified key as the affine coordinates of its point. The certificate is verified inside the standardized circuit (§6.5), which makes this compression wire format even though the certificate itself never travels: an implementation compressing differently produces proofs no other implementation can verify — or, worse, a per-client proof shape, the fingerprinting §6.5 exists to prevent. The agora sits inside the signed message, not merely alongside the signing request, so an epoch certificate cannot be replayed into another agora the member belongs to (§16.1); the leading field domain is what keeps this certificate and the migration certificate (§9.3) unforgeable for each other despite sharing a signing key, and arity closes what a movable boundary once threatened — every input is exactly one field element.
...
The transparency log is opt-in (§10.1), so it cannot be the only advance signal. The authoritative signal, for every agora, is the **signed boundary bulletin itself** (§11, proposal 0024): one signed object announces the boundary and equips the member for it, so there is no window in which a member knows the epoch advanced but cannot yet act in it, and no separate advance statement to secure — the statement 0024 subsumed was the degenerate bulletin. The local-clock maximum remains the backstop, and a member acts on whichever signal arrives first, exactly as above. This matters most where it is least convenient: revocation advances the epoch immediately (§11), and the agoras most likely to decline a log — the most existence-sensitive ones — are also the ones that most need prompt revocation to take effect. An early advance therefore never depends on a mechanism an agora may have opted out of.
...
The payload `m` is the canonical payload compressed to a field element by the same Poseidon instance. The deterministic nonce is an obligation on the signer — it costs the circuit nothing, keeps vectors byte-reproducible, and forecloses nonce reuse. Jubjub's cofactor is 8, and the equation is stated over the prime-order subgroup: subgroup membership of `R` and `PK` is part of the statement, and a witness point off the subgroup must fail to prove. The 32-byte key and 64-byte signature are the widths the provisional scheme already occupies, so nothing above the primitive moves when it retires.

The revocation-set root and migration-spend root are public inputs alongside the accumulator root; a verifier accepts a routine proof only against the current epoch's three roots. Both sets are **gap trees** (proposal 0035): the set's keys, truncated to a 253-bit ordering domain, sorted, with sentinels 0 and 2²⁵³−1 closing the ends; each consecutive pair `(low, high)` is a leaf `Poseidon(GAP, low, high)` in a positional tree of the protocol depth (§5.2). Absence of a key is a positive statement — inclusion of a gap strictly containing its truncation, both comparisons in-statement — which a present key can never satisfy, since its own truncation is what some gap boundary holds. Soundness is unconditional; a 253-bit truncation collision (~2⁻²⁵³) can cost only availability, a bound §15 records. The two non-membership clauses are what make §5.2's definition of a current credential a proven fact rather than a verifier's unaided obligation.

**Only the last line varies by action, and it varies inside one derivation** (proposal 0035):
...
context:  the action's identifier, entered by the encoding rules of §6.5
```

The tag is both a public input and an absorbed element, so one action's output cannot replay as another's, and every ordinary proof carries the same public inputs — §6.5's uniform shape made structural. Verification access (tag 4) derives nothing (proposal 0019); its output is constrained to the zero constant rather than left floating. The key choice is where the durability argument above takes effect: authorship derives from the epoch key so attribution expires with it; vouching, policy approval, and migration rest on `sk_cred`, because each is a count and a count cannot rest on a key the member can mint twice — migration's spend deriving under its own domain, `Poseidon(SPEND, sk_cred, leaf, agora_id)`, keyed by the leaf it consumes.
...
The migration certificate's signed message is canonical for the same reason the epoch certificate's is (§9.1): the Poseidon compression `Poseidon(MIGRATION_CERT, agora_id, pk_root_new.x, pk_root_new.y)`, with `MIGRATION_CERT` from the field-domain registry, the `agora_id` by the identifier rule (§6.5), and `pk_root_new` as the affine coordinates of its point. It is verified inside the proof, so this compression is wire format even though the certificate never travels: every implementation must compute the same field element as the circuit, or its proofs verify nowhere.
...
The same fork evidence exists on the member channel: the boundary bulletin (§11) is signed by a separate per-agora statement key and, where a log exists, embeds the latest signed head — so two validly signed bulletins for one epoch with different content are portable proof of a fork even for an agora that declined the log, and a bulletin's roots are cross-checkable against the public log with no extra fetch (proposal 0024).
...
The boundary broadcast carries more than the tag key: it is the members' distribution channel for everything the new epoch fixed — the epoch's canonical roots (proposal 0020), both exclusion sets whole, `K_tag`, and the witness-service key `K_witness` (§5.2, proposal 0025) — so that every remaining member can refresh witnesses and act without a bootstrap dependency on the member-gated services of §7, which a member could not satisfy at a boundary where anything changed without the very material being distributed. It is also a member's only source for the current epoch's roots — there is no lookup to gate or to leak (§5.2). The sets travel whole rather than as deltas: a delta presumes an earlier copy, and a member admitted at that very boundary has none — they would start life unable to compute the absence witnesses their first proof requires. The delivery cut is the security property: what reaches remaining members and not the revoked one is exactly this broadcast.

**The bulletin is a signed operator statement** (proposal 0024). It is meant to be cached, relayed peer-to-peer, and fetched through infrastructure the member does not trust, so it carries its own authenticity — object security, not a property of one hop. Its canonical bytes — the bulletin domain tag leading, the `agora_id` inside the signed message (§16.1), every field length-framed, the sets in ascending order — are signed by a per-agora **operator statement key**: distinct from all member material, because the signature makes the *operator* non-repudiable and says nothing about members, and distinct from the log-head key (§10.1), so a log key disclosed for public auditing reveals nothing about the member-gated channel. A member pins the statement key at admission, alongside the `agora_id` itself, and accepts a bulletin only if the signature verifies **and** the epoch is strictly greater than their current one. Monotonicity is the whole freshness rule — sufficient because the sets arrive whole: a member offline for several boundaries applies the latest bulletin alone and is current — and it is what keeps a replayed pre-revocation bulletin from holding a targeted verifier at stale roots, so revocation's forced boundary survives transit, not merely the machine. Two validly signed bulletins for one epoch with different content are portable proof of a fork, carryable to anyone holding the statement key — §10.1's equivocation check extended to agoras that declined the log, and the reason per-member authenticators were rejected outright: a per-member MAC is precisely the tool for undetectable per-member forking. Where the agora keeps a log, the bulletin embeds the latest signed log head, binding the member-gated artifact to the public one with no extra fetch. The cost is named rather than hidden: a signature is a name, and a leaked bulletin proves the agora exists (§3) — 0024 argues why signing wins that trade on a channel only members receive.
