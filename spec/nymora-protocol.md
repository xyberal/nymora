...
Second, **accumulator depth is a protocol constant, not an agora parameter** (proposal 0030). One depth is fixed network-wide at protocol-version standardization, because the membership path lives inside the standardized circuit (§6.5) and its length *is* the depth: a per-agora depth would be a per-agora proof shape — the fingerprinting §6.5 exists to prevent. The numeric value is settled with the real circuit, informed by measured constraint counts (proposal 0001's discipline). What no constant changes is that capacity consumption is permanent: migrated predecessors and revoked members consume capacity forever, and since planned migration is the expected path for a routine device change, consumption tracks device churn rather than recruitment.

**Exhaustion is terminal for the class.** A policy class whose accumulator is full admits no further leaf under the current protocol version — no new admission and, because migration consumes capacity, no routine device change either, which is the worse of the two. No mechanism compacts, extends, or re-accumulates a tree: leaves cannot be withdrawn (above), and a re-accumulation ceremony would change every member's witness and the class's root lineage at once — a protocol-version event requiring its own proposal, not an operational recovery. The mitigation is to fix the constant generously, where the cost is logarithmic: depth 32 accommodates roughly four billion leaves at thirty-two siblings per witness. It must be sized for the largest class the network will ever host, because every class in every agora lives inside it and there is no second chance later.
