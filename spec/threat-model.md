...
What it costs to escape: nothing rotates `sk_cred`, and migration deliberately preserves it, so a member who believes it has leaked has only §9.3's Path 2 — quorum revocation and re-vouching, surrendering tenure, vouch count, and tier. That is the full lost-device penalty paid without a lost device. Whether an agora treats a suspected key leak as grounds for it is a §5.3 policy judgement, and one worth making before it is needed.
